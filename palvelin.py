...
async def init(app):
    try:
        ...
        if homed: ## Prepare motor parameters for operation
            ...
            # res1 = await clients.client_right.write_register(address=config.ANALOG_INPUT_CHANNEL,value=2,slave=config.SLAVE_ID)
            # res2 = await clients.client_left.write_register(address=config.ANALOG_INPUT_CHANNEL,value=2,slave=config.SLAVE_ID)

            # (position_client_right, position_client_left) = await get_modbuscntrl_val(clients, config)

            await clients.client_left.write_register(address=7188, value=15000)
            ...
            # await clients.client_right.write_register(address=config.ALTERNATIVE_COMMAND, value=2, slave=config.SLAVE_ID)
            # await clients.client_left.write_register(address=config.ALTERNATIVE_COMMAND, value=2, slave=config.SLAVE_ID)

            # Enable motors
            # await clients.client_right.write_register(address=config.IEG_MODE, value=130, slave=config.SLAVE_ID)
            # await clients.client_left.write_register(address=config.IEG_MODE, value=130, slave=config.SLAVE_ID)
